...
remotes::install_github("rstudio/ggsave")
...
#############################################################################################Interaction of IHL and PRS for 9 TG-associated genes at random
### select 9 genes at random for 100 times
set.seed(1234)
TG_sample1 <- replicate(100, {
...
### repeat the main analysis for 100 times with 100 sets of TG-associated genes at random
interaction1 <- NULL
association1 <- NULL
for (i in 1:100) {
  TG_new <- data[,c(1,767:769,774:783,785:787,791,803,827:832,841:846,852:858,876,which(substring(names(data),4) %in% TG_sample1[,i]))]
  ...
  # association between PRS for 9 TG genes at random and TG
  lm1 <- lm(formula = log10(Triglyc) ~ PRS + Age + SEX + T2D + C1_1KGP3 + C2_1KGP3 + C3_1KGP3 + C4_1KGP3 + C5_1KGP3 + C6_1KGP3
  ...
  print(sum1)
  coef1 <- data.frame(sum1$coefficients)
  association1 <- c(association1, coef1$Estimate[2])
  ...
  coef_interaction <- data.frame(sum_interaction$coefficients)
  confint_interaction <- data.frame(confint(lm_interaction))
  interaction1 <- c(interaction1,coef_interaction$Estimate[nrow(coef_interaction)-1])
}
...
for (i in 1:n_bootstrap) {
  sample_data <- sample(association1, size = length(association1), replace = TRUE)
  bootstrap_means[i] <- mean(sample_data)
}

# mean association
mean_value <- mean(bootstrap_means)
exp(mean_value) # 1.216
...
### get mean value of the interaction term and corresponding CI
# Bootstrap sampling
set.seed(123)  
for (i in 1:n_bootstrap) {
...
################################################################################Interaction of IHL and PRS for 9 TG genes randomly selected from the remaing 3 categories
# select 100 set of 9 TG genes at random
TG_other <- subset(TG,TG$`Role in TG metabolism` %in% c(0,3,4))

set.seed(1234)
TG_sample2 <- replicate(100, {
  sample(TG_other$SNP, 9, replace = FALSE)
})
...
for (i in 1:100) {
  TG_new <- data[,c(1,767:769,774:783,785:787,791,803,827:832,841:846,852:858,876,which(substring(names(data),4) %in% TG_sample2[,i]))]
  TG_new <- subset(TG_new,is.na(TG_new$liverfat_adj)==FALSE) 
  TG_new <- subset(TG_new, TG_new$liverfat_adj != -999) 
  TG_new <- subset(TG_new,is.na(TG_new$batch)== FALSE) 
  TG_new <- TG_new[complete.cases(TG_new[,40:57]),] 
  ...
  lm1 <- lm(formula = log10(LDL) ~ PRS + Age + SEX + T2D + C1_1KGP3 + C2_1KGP3 + C3_1KGP3 + C4_1KGP3 + C5_1KGP3 + C6_1KGP3
            +C7_1KGP3 + C8_1KGP3 + C9_1KGP3 + C10_1KGP3 + med_LP + NIT_alcoholtot,data = TG_new)
  sum1 <- summary(lm1)
  coef1 <- data.frame(sum1$coefficients)
  beta_LDL <- c(beta_LDL,coef1$Estimate[2])
  
  lm1 <- lm(formula = log10(HDL) ~ PRS + Age + SEX + T2D + C1_1KGP3 + C2_1KGP3 + C3_1KGP3 + C4_1KGP3 + C5_1KGP3 + C6_1KGP3
            +C7_1KGP3 + C8_1KGP3 + C9_1KGP3 + C10_1KGP3 + med_LP + NIT_alcoholtot,data = TG_new)
  sum1 <- summary(lm1)
  coef1 <- data.frame(sum1$coefficients)
  beta_HDL <- c(beta_HDL,coef1$Estimate[2])
}
